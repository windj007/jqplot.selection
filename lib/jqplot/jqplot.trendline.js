...
(function($) {
    ...
    function polyRegression(x_src, y_src, power) {
        var x_norm = x_src[0];
        var a_src = new Array(x_src.length);
        for (var i = 0; i < x_src.length; i++) {
            var cur_x = x_src[i] - x_norm;
            var row = new Array(power + 1);
            for (var j = 0; j < row.length; j++)
                row[j] = Math.pow(cur_x, j);
            a_src[i] = row;
        }
        
        var A = $M(a_src);
        var AT = A.transpose();
        var Y = $M([y_src]).transpose();
        var w = AT.x(A).inv().x(AT).x(Y);
        return A.x(w);
    }
    ...
    function fitData(data, typ) {
        ...
        else if (type == 'moving_avg3' || type == 'moving_avg14') {
            var window = type == 'moving_avg14' ? 14 : 3;
            
            ret = [1, 1];
            res = 0;
            for ( var i=0; i<window; i++) {
                res += y[i];
                ypred.push([x[i], res / (i + 1)]);
            }
            for ( var i=window; i<x.length; i++){
                res = res - y[i-window] + y[i]; // move window
                ypred.push([x[i], res / window]);
            }
        }
        else if (type == 'poly') {
            ret = [1, 1];
            var power = 5;
            var trend = polyRegression(x, y, power);
            for ( var i=0; i<trend.rows(); i++){
                ypred.push([x[i], trend.e(i+1, 1)]);
            }
        }
